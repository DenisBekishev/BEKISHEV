...
double* veсtorNevyazki(double** matrix_full, double* matrix_answers, double* answers, int n) {
...
int main() {
	...
	cout << endl;
	cout << "Вектор невязки: ";
	double* vectorNevyaz = veсtorNevyazki(matrix_full_copy, matrix_answers_copy, answers, n);
	for (int i = 0; i < n; i++) {
		cout << vectorNevyaz[i] << " ";
	}
	...
	cout << endl << "Норма невязки: " << norma << endl;

	double* new_answers = find_x(matrix_full_copy, answers, n);
	cout << endl << "Новый X:" << endl;
	for (int i = 0; i < n; i++) {
		cout << "X" << i + 1 << " = " << new_answers[i] << " ";
	}
	cout << endl << "Относительная погрешность: " << pogreshnost(answers, new_answers, n);
