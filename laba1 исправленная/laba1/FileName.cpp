...
int main() {
    ...
    for (int i = 0; i < n; i++) {
        double numerator = abs(x[i] - x_new[i]);
        double denominator = abs(x[i]);
    ...
    double delta = maxNumerator / maxDenominator;

    cout << "Relative error using the corrected method: " << delta << endl;
